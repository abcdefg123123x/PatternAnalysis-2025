...
### Network Architecture
  - **Embedding Network (Siamese backbone)**
    ...
    *Image generated by DeepSeek (more information in [References](#references))*
    ![Siamese Network](https://github.com/user-attachments/assets/766b2719-036e-4cbd-9cbd-ec9bb376d804)

    - Base model: ResNet 34 backbone for general image feature extraction.
...
1. **Strong Data Augmentation for Malignant Images**
   ...
   *Strong augmentations produced by ChatGPT (more information in [References](#references))*
...
3. **Moderate Augmentation for Benign Images**
...
⋮
Classifier Epoch 25/25: 100%|██████████| 725/725 [00:01<00:00, 604.11it/s, acc=0.996, loss=0.0132]
...
python predict.py
```
Note that `predict.py` is intended for demonstration purposes only on the saved, trained models to show they work for inferencing. It uses a sample subset of the dataset (10% of all images) for evaluation, drawn randomly from the validation dataloader. The sampled images may or may not have been seen during training, so results on this subset do not necessarily reflect performance on a dedicated test set (the test set was already done in `train.py` as mentioned before). Each run may produce a different set of images due to random sampling.  
...
The accuracies rise sharply during the initial epochs, improving from around 99%-99.3% in epoch 1 to approximately 99.5% by epoch 5. After this increase, both training and validation accuracies stabilise, consistently remaining between 99.4% and 99.7% for the remainder of training. This indicates that the model quickly converged and maintained strong, stable performance without significant signs of overfitting.
...
## References 
ISIC 2020 JPG 224x224 Resized: https://www.kaggle.com/datasets/nischaydnk/isic-2020-jpg-224x224-resized

DeepSeek generated both the visualisations of the Siamese extractor and binary classifier. It was prompted with code from `modules.py` to generate an image of how the entire network functions.  

ChatGPT was used to generate information about the equation used by `TripletMarginLoss()` in Python. 

ChatGPT was used to produce stronger augmentations given the basic benign augmentations (acknowledged in `dataset.py` as well). It was initially intended for both classes to use the same basic augmentations, but then it was decided to use stronger augmentations for the malignant class to improve test accuracy. 

[1]: Sarıgöz, Y. (2022, March 24). Triplet Loss - Advanced Intro. Qdrant. https://qdrant.tech/articles/triplet-loss/ 
